...
simple_tem_sys_prompt = f"""
    你是一个ST项目需求逆向提取专家，你需要根据提供的prg，根据prg的代码逻辑，逆向出一个需求。
    以下是ST项目结构的介绍：
    项目结构：
    -  前言：对于ST项目而言，它们的项目结构比较简单，往往几个文件就是一个完整的项目。下面将阐述具体的文件和对应的功能：
    -  文件与模块：
...
mod1_tem_sys_prompt = f"""
    ...
    注意：要避免照着代码逻辑描述，那不是逆向需求，那只是把代码逻辑复述了一遍而已。你要像真正地提出一个需求那样提出，想象最终的代码可能需要引用到那些数据结构、全局变量和功能块，但是你只需要描述出这个prg块想达到的效果。
"""

def project_struct_generate(dut_json, gvl_json, st_json, prg_json):
    code_messages = [{"role": "system", "content":mod1_tem_sys_prompt}]
    ...
    code_messages.append({"role": "user", "content": simple_task_prompt})
